...
public class AccountsService {
	...
	public String AddAdmin(String signUpRequest) {
		...
		try {
			...
			Map <String, String> resultStatus = signUpProcess.AddAdminAndCompany(newAdmin);
	...
	public String AddDriver(String addRequest) {
		...
		try {
			...
			Map<String, String> resultStatus = addDriverProcess.AddDriver(requestingAdmin, newDriver);
			
			if(resultStatus.get("status").equals(ResponseStatus.Failed.toString())) {				
				if(resultStatus.containsKey("errorMssg")) {
					response.put("errorMssg", resultStatus.get("errorMssg"));
