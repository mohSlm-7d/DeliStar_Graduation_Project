	...
	public Map<String, String> AddAdminAndCompany(Admin newAdmin) {
		try {	
			...
			if(newCompany.getCompanyName() == null) {
				...
				result.put("errorMssg", "The company name is invalid. Company name must only contain characters and spaces and can contain at most 25 characters.");
				return result;
			}
			...
			if(newAdmin.getAdminName() == null) {
				...
				add.CloseConnection();
				result.put("status", ResponseStatus.Failed.toString());
				result.put("errorMssg", "The admin name is invalid. Admin name must only contain characters and spaces and can contain at most 25 characters.");
				return result;
			}
			...
			boolean validPassword = ValidationUtility.validatePassword(newAdmin.getAdminPassword());
			if(!validPassword) {
				add.CloseConnection();
				result.put("status", ResponseStatus.Failed.toString());
				result.put("errorMssg", "The password is invalid. Password contain at least of 8 characters and maximum 20.");
				return result;
			}
	...
	public Map<String, String> AddDriver(Admin requestingAdmin, Driver newDriver) {
		try {
			...
			if(newDriver.getDriverName() == null) {
				add.CloseConnection();
				result.put("status", ResponseStatus.Failed.toString());
				result.put("errorMssg", "The driver name is invalid. Driver name must only contain characters and spaces and can contain at most 25 characters.");
				return result;
			}
			...
			newDriver.setDriverPassword(""+newDriver.getDriverPassword().hashCode());
