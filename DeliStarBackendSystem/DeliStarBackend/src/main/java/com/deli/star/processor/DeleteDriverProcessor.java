...
public class DeleteDriverProcessor {
	public Map<String, String> DeleteDriver(Admin requestingAdmin, Driver driverToDelete) {
		...
		try {
			...
			// Checking if there are any assigned orders to this driver before deleting them.
			...
			if(driverAssignedOrders != null && driverAssignedOrders.size() > 0) {
				System.out.println("This driver to be deleted is assigned to deliver existing orders!!!");
				result.put("status", ResponseStatus.Failed.toString());
				String errorMssg = "The driver you want to delete is already assigned to deliver existing orders. Try re-assigning the following orders to different drivers. Assigned orders numnbers:[ " ;
